...
export class HydrateSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Select the language model to use. Ensure you have the corresponding API key set.",
			)
		...
		new Setting(containerEl)
			.setName("License key")
			.setDesc(
				"Enter your license key for paid subscriptions. Leave empty for free tier.",
			)
			.addText((text) =>
				text
		...
		const apiKeysDesc = containerEl.createEl("p", {
			text: "Configure your API keys. These are sent securely but never stored permanently.",
		...
		formatRegistrySection.createEl("p", {
			text: "Manage reusable templates and context snippets.",
		...
		new Setting(mcpHeadingEl).setName("Servers").setHeading();
		...
		mcpServersSection.createEl("p", {
			text: "Configure servers to extend capabilities. Only available with paid plans.",
			cls: "setting-item-description",
		});
		...
		new Setting(containerEl)
			.setName("Custom path")
			.setDesc(
				"Comma-separated paths to add to path. Needed if Obsidian can't find executables.",
			)
			.addText((text) =>
				text
		...
		if (!this.plugin.settings.enableRemoteEmbeddings) {
			...
			noticeEl.createEl("p", {
				text: "💡 enable embeddings for context search. Requires an API key.",
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Use an API endpoint to generate embeddings. Requires configuration below.",
			)
			.addToggle((toggle) =>
				toggle
		...
		if (this.plugin.settings.enableRemoteEmbeddings) {
			new Setting(containerEl)
				...
				.setDesc(
					"Full URL of the openai-compatible embedding API endpoint",
				)
				.addText((text) =>
					text
						.setPlaceholder(
	...
	renderFormatRegistryList(containerEl: HTMLElement) {
		...
		if (entries.length === 0) {
			containerEl.createEl("p", {
				text: "No entries defined. Click 'add new entry' to create one.",
	...
	renderRulesRegistryList(containerEl: HTMLElement) {
		...
		if (rules.length === 0) {
			containerEl.createEl("p", {
				text: "No rules defined. Click 'add new rule' to create one.",
	...
	renderMCPServersList(containerEl: HTMLElement) {
		...
		if (servers.length === 0) {
			containerEl.createEl("p", {
				text: "No servers configured. Add a server to extend capabilities.",
				cls: "hydrate-empty-list-message",
			});
			return;
		}
	...
	private async testServerConnection(server: MCPServerConfig) {
		if (!this.plugin.mcpManager) {
			new Notice("Manager not available.");
			return;
		}
	...
	private async loadRegistrationStatus(statusDiv: HTMLElement) {
		try {
			...
			quotaList.createEl("li", {
				text: `Quota resets: ${new Date(
					quota.next_reset,
				).toLocaleDateString()}`,
			});
			quotaList.createEl("li", {
				text: `Remaining: ${
					quota.max_reregistrations - quota.registration_count
				}`,
			});
		} catch (error) {
			statusDiv.empty();
